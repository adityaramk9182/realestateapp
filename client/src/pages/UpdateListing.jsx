...
const UpdateListing = () => {
  ...
  React.useEffect(()=>{
    const fetchSinglelisting = async(req, res, next) => {
        try{
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
        <h1 className='font-semibold text-center text-3xl py-7'>Update Listing</h1>
        <form className='flex flex-col sm:flex-row gap-4' onSubmit={handleFormSubmit}>
          <div className='flex flex-col gap-4 flex-1'>
            ...
            <div className='flex gap-5 flex-wrap'>
              <div className='flex items-center gap-2'>
                <input type="number" id='bedRooms' className='border p-3 rounded-lg border-gray-300 outline-none' value={formData.bedRooms} max='10' min='1' onChange={handleOnChange} required/>
                <span>Beds</span>
              </div>
              <div className='flex items-center gap-2'>
                <input type="number" id='bathRooms' className='border p-3 rounded-lg border-gray-300 outline-none' value={formData.bathRooms} max='10' min='1' onChange={handleOnChange} required/>
                <span>Baths</span>
              </div>
              <div className='flex items-center gap-2'>
          ...
          <div className='flex flex-col flex-1 gap-4'>
            ...
            <button className='p-3 border rounded-lg text-center text-[#FCF6F5] bg-[#2BAE66] uppercase font-semibold hover:opacity-95 disabled:opacity-85' type='submit' disabled={loading || uploading}>{loading ? 'Updating...' : 'Update Listing'}</button>
          </div>
        </form>
    </main>
  )
